...
# This file contains the monitored navigation with recovery behaviours
# state machine
...
# The cild termination callback decided when the concurrence container
# should be terminated, bases on the outcomes of its children. When it
# outputs True, the container terminates and the concurrence container
# outcome callback is called
...
# container's children into an outcome for the concurrence container
# itself
...
# Move base + recovery behaviour. The number of move_base fails is sent
# from the move_base action state to the move_base recovery state.
def move_base_sm():
    sm = smach.StateMachine(
        outcomes=['succeeded', 'failure', 'preempted'], input_keys=['goal_pose'])

    with sm:

        sm.userdata.n_move_base_fails = 0
        mv_base_act = smach_ros.SimpleActionState('move_base',
                                                  MoveBaseAction,
...
# adds the bumper and battery monitors running concurrently, and the
# bumper recovery as an extra state in the navigation state machine
def navigation():
        ...
        sm = smach.StateMachine(outcomes=['succeeded',
                                ...
                                input_keys=['goal_pose',
                                            'going_to_charge'])
        with sm:
            # move_base state machine running in parallel with the bumper and
            # battery monitor states. The input key 'goal_pose' is used to
            # build the new move_base goal, and the input key 'going_to_charge'
            # is used to define the behaviour of the battery monitor
            monitored_move_base = smach.Concurrence( outcomes=['bumper_pressed',
                                                               'battery_low',
                                                               'succeeded',
                                                               'failure'],
            ...
            # adds the bumper recovery behaviour. This state needs to be on the same
            # level as the concurrence container that has the bumper monitor so that
            # the monitor can be restarted after the bumper is recovered.
