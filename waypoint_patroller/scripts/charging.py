...
# This file contains the monitored docking with recovery behaviours state
# machine


# Calls the docking service. will be changed to call a docking action when one
# is available
...
# undocks. Should be changed to used the undock service (eventually action,
# when it is ready) instead of what is here right now
...
# Docking + bumper recovery behaviour. The input key 'going_to_charge' defines
# the battery monitor state results. In this case, it trminates when the battery
# life goes above the charged battery treshold
def monitored_docking():
    conc = smach.Concurrence(
        outcomes=['bumper_pressed', 'succeeded', 'failure'],
    ...
    with conc:
        ...
        unmonitored_dock = smach.StateMachine(outcomes=['succeeded',
                                                        'failure',
                                                        'preempted'],
                                              input_keys=['going_to_charge'])
...
# level as the concurrenc container that has the bumper monitor so that
# the monitor can be restarted after the bumper is recovered.
def dock_and_charge():

    docking_sm = smach.StateMachine(outcomes=['succeeded', 'failure'],
                                    input_keys=['going_to_charge'])
