...
# tresholds. It is used so that the battery_monitor can read the
# tresholds, which can be dynamically reconfigured by
# long_term_patroller.py
from parameter_store import ParameterStore


# this file has the monitor states that will run in parallel with other 
# behaviours, using a smach concurrence container. These states subscribe to a
# given topic and remain active until their callback returns False.  When that
# happens they terminate and return outcome 'invalid'.
...
def battery_monitor():
    state = smach_ros.MonitorState("/battery_state",
                                   BatteryState,
                                   battery_cb,
